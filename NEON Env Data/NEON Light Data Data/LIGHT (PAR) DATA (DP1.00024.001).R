...
output_dir <- file.path(
  ...
  "NEON Code",
  "siteaverages_DP1_00024_with_units"
)
...
for(site in sites){
  ...
  site_outfile <- file.path(
    output_dir,
    paste0(code, ".", site, ".AVERAGE_ALL_VARIABLES_WITH_UNITS.csv")
  )
  ...
  if(length(all_rows) == 0){
    cat("\nNo data downloaded for site:", site, "\n")
    next
  }
...
if(length(master_all_sites) > 0){
  ...
  master_outfile <- file.path(
    output_dir,
    paste0(code, ".MASTER_ALL_SITES_AVERAGE_ALL_VARIABLES_WITH_UNITS.csv")
  )
